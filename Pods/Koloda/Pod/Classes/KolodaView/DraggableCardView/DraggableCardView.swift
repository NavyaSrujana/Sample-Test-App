...
public class DraggableCardView: UIView, UIGestureRecognizerDelegate {
    ...
    func panGestureRecognized(_ gestureRecognizer: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            updateOverlayWithFinishPercent(percent: percentage, direction:dragDirection)
            if let dragDirection = dragDirection {
                //100% - for proportion
                delegate?.card(card: self, wasDraggedWithFinishPercentage: min(fabs(100 * percentage), 100), inDirection: dragDirection)
            }
    ...
    private var dragDirection: SwipeResultDirection? {
        ...
        let normalizedDragPoint = dragDistance.normalizedDistanceForSize(size: bounds.size)
        return directions.reduce((distance:CGFloat.infinity, direction:nil)) { closest, direction in
            let distance = direction.point.distanceTo(point: normalizedDragPoint)
    ...
    private var dragPercentage: CGFloat {
        ...
        let normalizedDragPoint = dragDistance.normalizedDistanceForSize(size: frame.size)
        let swipePoint = normalizedDragPoint.scalarProjectionPointWith(point: dragDirection.point)
        ...
        if !rect.contains(swipePoint) {
            ...
            let centerDistance = swipePoint.distanceTo(point: .zero)
            ...
            return rect.perimeterLines
                        .flatMap { CGPoint.intersectionBetweenLines(line1: targetLine, line2: $0) }
                        .map { centerDistance / $0.distanceTo(point: .zero) }
                        .min() ?? 0
        }
    }
    ...
    private func updateOverlayWithFinishPercent(percent: CGFloat, direction: SwipeResultDirection?) {
        ...
        overlayView?.updateWithProgress(percentage: progress)
    }
    
    private func swipeMadeAction() {
        let shouldSwipe = { direction in
            return self.delegate?.card(card: self, shouldSwipeInDirection: direction) ?? true
        }
    ...
    private func animationPointForDirection(direction: SwipeResultDirection) -> CGPoint {
        ...
        let retPoint = animatePoint.screenPointForSize(screenSize: screenSize)
        return retPoint
    }
    ...
    private func swipeAction(direction: SwipeResultDirection) {
       ...
       // delegate?.card(self, wasSwipedInDirection: direction)
        ...
        translationAnimation?.toValue = NSValue(cgPoint: animationPointForDirection(direction: direction))
    ...
    func swipe(direction: SwipeResultDirection) {
        if !dragBegin {
            ...
            swipePositionAnimation?.toValue = NSValue(cgPoint:animationPointForDirection(direction: direction))
            ...
            swipeRotationAnimation?.toValue = CGFloat(animationRotationForDirection(direction: direction))
